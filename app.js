...
let [num_cpus, adtag, port, secrets] = [
	"num_cpus",
	"adtag",
	"port",
	"secrets",
...
if (!num_cpus) num_cpus = require("os").cpus().length;
...
function requireFromURL(url) {
	...
	return new Promise(function (accept, reject) {
		...
		vm.runInNewContext(
			data.toString(),
			{ require, Buffer, module: { exports: load } },
			url,
		);
		return load;
	});
...
if (cluster.isMaster) {
	...
	function onConnection(core, options) {
		...
		tracker[id] = options;
		ongoingConnections++;
		if (!stats[options.address]) stats[options.address] = 0;
		...
		if (p === "/clients") {
			res.write(
				"<html><h1>Dear " +
					...
					", Welcome; Here are the clients:</h1>",
			);
			res.end(
				`<h2>Statistics</h2><div>totalBytesRead: ${totalBytesRead}</div><div>totalBytesWritten: ${totalBytesWritten}</div><div>totalConnections: ${totalConnections}</div><div>ongoingConnections: ${ongoingConnections}</div><h2>Current clients:</h2><div>${Object.keys(
					stats,
				)
					.map((ip) => ip + ":" + stats[ip])
					.join("</div><div>")}</div></html>`,
			);
			return;
		}
		res.end(
			`<html><h1>This website is under construction...</h1><div>Comeback later please.</div></html>`,
		);
		return;
	});

	requireFromURL(
		"https://raw.githubusercontent.com/MTProto/MTProtoProxy/master/mtprotoproxy.js",
	).then(function ({ MTProtoProxy }) {
		let telegram = new MTProtoProxy({
			secrets,
			httpServer,
			async enter(options) {
				process.send(
					Object.assign({ eventName: "connection" }, options),
				);
				return adtag;
			},
			leave(options) {
